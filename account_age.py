...
def find_date(user_id):
    """Find approximate creation date for a user ID through linear interpolation"""
    try:
        user_id = int(user_id)
        ...
        print(f"Error in find_date: {e}")
        return None

def creation_date(user_id):
    """Return a formatted string with the estimated account creation date and relative time"""
    try:
        result = find_date(user_id)
        if result:
            comparison, date = result
            approx = "تقریباً " if comparison == "~" else "بعد از " if comparison == ">" else "قبل از "
            return f"{approx}{date.strftime('%Y-%m-%d')} ({relative_time(date)})"
        return "نامشخص"
    except Exception as e:
        print(f"Error in creation_date: {e}")
        return "نامشخص" 